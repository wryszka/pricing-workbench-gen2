...
## Governance tab — IF-FAILS notes

The pricing workbench tabs (Data Ingestion, Model Factory, Feature Store, Governance) are outside the optimisation Acts above. If you visit the Governance tab in the room:

**"Generate pack" PDF button hangs** — this is a **known serverless environment stall** (documented in DEPLOY.md). The button triggers a live PDF render which can hang indefinitely on the serverless JVM cold path.
- **Fallback:** close the spinner, switch to the **Pack History** list — the pre-seeded packs (one per champion, generated by `generate_governance_packs` during `full_build`) are all there. Click any pack to show the PDF inline.
- **Reproduce view fallback:** the Reproduce tab lets you show the pack *was* generated from a known model version + data snapshot — the governance story lands without a live render.
- SAY: "The packs you see here were generated automatically at deploy time — this is the artefact a regulator sees. The live-render button re-runs it on demand; we'll skip the wait and show you the pre-built one."

**Cold start for first serving call after a deploy: ~45s** — endpoints are scale-to-zero. If a quote or agent call hangs on first click, the endpoint is warming. Subsequent calls are sub-second. Warm before the room if possible (`Before the room` checklist above).
